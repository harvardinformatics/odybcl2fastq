...
rule demultiplex_10x_cmd:
    ...
    shell:
        ...
        cmd+="/usr/bin/time -v cellranger{config[atac]} mkfastq --run=/sequencing/source/{config[run]} --samplesheet={sample_sheet_path} --output-dir=/sequencing/analysis/{config[run]}{config[suffix]}/fastq --localmem=\$((8*\$(ulimit -m)/10000000)) --loading-threads=\$((SLURM_JOB_CPUS_PER_NODE/4)) --writing-threads=\$((SLURM_JOB_CPUS_PER_NODE/4)) --processing-threads=\$SLURM_JOB_CPUS_PER_NODE --localcores=\$SLURM_JOB_CPUS_PER_NODE --barcode-mismatches=0 || exit_code=\$?\n"
...
rule count_10x_cmd:
    ...
    shell:
        ...
        cmd+="/usr/bin/time -v cellranger{config[atac]} count --id={wildcards.sample} $transcriptome --sample={wildcards.sample} --fastqs=$fastq_path --localmem=\$((8*\$(ulimit -m)/10000000)) --localcores=\$SLURM_JOB_CPUS_PER_NODE || exit_code=\$?\n\n"
